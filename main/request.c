...
//         reading = DHT11_read();
//         freeaddrinfo(res);
//         float far = (reading.temperature * (9/5)) + 32;
//         char fmt_str[255];
//         sprintf(fmt_str, REQUEST_POST_MEASUREMENT,reading.temperature, reading.humidity);
//         ESP_LOGI(TAG,"request: %s", fmt_str);
//         if (write(s, fmt_str, strlen(fmt_str)) < 0) {
//             ESP_LOGE(TAG, "... socket send failed");
//             close(s);
//             vTaskDelay(4000 / portTICK_PERIOD_MS);
//             continue;
